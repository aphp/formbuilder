...
import {faAngleDown, faAngleRight, faCloudArrowUp, faPlusCircle, faTrash} from '@fortawesome/free-solid-svg-icons';
...
export class TableComponent extends LfbArrayWidgetComponent implements OnInit, AfterViewInit, DoCheck, OnChanges {
  ...
  constructor(private elRef: ElementRef, private cdRef: ChangeDetectorRef, private extService: ExtensionsService, private modalService: NgbModal) {
    super();
  }
  ...
  createValueSetButtonLabel: string = null;
  ...
  ngOnInit() {
    ...
    this.createValueSetButtonLabel = widget.createValueSetButtonLabel;
  ...
  ngAfterViewInit() {
    ...
    subscription = this.formProperty.searchProperty(this.keyField).valueChanges.subscribe(() => {
  ...
  addItemWithAlert(popoverRef) {
    ...
    if (!lastItem || !Util.isEmpty(lastItem.value)) { // If no lastItem or be not empty.
      ...
      setTimeout(() => {
        ...
        const inputElement = this.getInputElementInTable(props.length - 1, 0);
        if (inputElement) {
          inputElement.focus();
        }
      });
  ...
  public focusNext() {
    const selector = `[name^="${this.id}"]`
    const tabbables = document.querySelectorAll(selector) as NodeListOf<HTMLElement>;
  ...
  createValueSet() {
    ...
    const modalRef = this.modalService.open(ValueSetCreateDlgComponent, {size: 'lg', scrollable: true, backdrop: 'static'});
    const concepts: CodeSystemConcept [] = [];
    for (const item of this.formProperty.value) {
      if (item?.valueCoding) {
        const codeSystemConcept: any = {
          code: item?.valueCoding?.code,
          display: item?.valueCoding?.display,
          system: item?.valueCoding?.system,
          score: item?.valueCoding?.__$score,
          initialSelected: item?.initialSelected
        };
        concepts.push(codeSystemConcept);
      }
    }
    modalRef.componentInstance.codeSystemConcepts = concepts;
    return modalRef.result.then((value) => {
      if (value) {
        this.formProperty.searchProperty('__$answerOptionMethods').setValue('value-set', false);
        this.formProperty.searchProperty('answerValueSet').setValue(value?.valueSetUrl, false);
        if(value?.initialCodingValue){
          this.formProperty.searchProperty('__$initialExpressionYesNo').setValue('value', false);
          this.formProperty.searchProperty('initial').setValue([{valueCoding: value.initialCodingValue}], false);
        }
      }
    });
  }

  isCreateValueSetButtonDisable() {
    return this.formProperty?.value?.some(elt => elt.display);
  }
}
