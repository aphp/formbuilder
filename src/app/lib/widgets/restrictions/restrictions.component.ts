...
export class RestrictionsComponent extends TableComponent implements OnInit {
  ...
  static optionsDef = {
    ...
    maxDecimalPlaces: {
      extUrl: 'http://hl7.org/fhir/StructureDefinition/maxDecimalPlaces',
      display: 'Max decimal places'
    }
  };

  static readonly stringOptions = RestrictionsComponent.getOptions(['maxLength', 'minLength', 'regex']);
  static readonly numberOptions = RestrictionsComponent.getOptions(['maxValue', 'minValue', 'maxLength', 'minLength']);
  static readonly decimalOptions = RestrictionsComponent.getOptions(['maxValue', 'minValue', 'maxLength', 'minLength', 'maxDecimalPlaces']);
  static readonly attachOptions = RestrictionsComponent.getOptions(['maxSize', 'mimeType']);
  static readonly referenceOptions = RestrictionsComponent.getOptions(['regex']);
  static readonly quantityOptions = RestrictionsComponent.getOptions(['maxDecimalPlaces']);
  static readonly urlOptions = RestrictionsComponent.getOptions(['maxLength']);
  static readonly openChoiceOptions = RestrictionsComponent.getOptions(['maxLength']);

  static typeToOptions = {
    decimal: RestrictionsComponent.decimalOptions,
    ...
    reference: RestrictionsComponent.referenceOptions,
    quantity: RestrictionsComponent.quantityOptions,
    url: RestrictionsComponent.urlOptions,
    'open-choice': RestrictionsComponent.openChoiceOptions
  };
  ...
  constructor(private restrictionOperatorService: RestrictionOperatorService,
    ...
    super(elementRef, cdr, extensionsService, ngbModal);
  }
  ...
  updateRelevantExtensions(extensions: fhir.Extension [], restrictions: any []) {
    ...
    Object.keys(RestrictionsComponent.optionsDef).forEach((opt) => {
      ...
      if (opt === 'maxLength') {
        ...
        // delete
        this.extensionsService.removeExtension(e => (e.value?.url && e.value.url === extUrl))
      }
    });
  }
