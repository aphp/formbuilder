...
export class AnswerOptionComponent extends TableComponent implements AfterViewInit, OnInit, OnDestroy {
  ...
  constructor(private elementRef: ElementRef, private cdr: ChangeDetectorRef,
              private formService: FormService, private extensionService: ExtensionsService, private ngbModal: NgbModal) {
  ...
  ngAfterViewInit() {
    ...
    if (repeatProp) {
      sub = repeatProp.valueChanges.subscribe((isRepeating) => {
        ...
        if (!this.initializing) {
          const firstCheckbox = this.selectionCheckbox.findIndex((e) => {
            return e
          });
