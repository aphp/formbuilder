...
export class SfFormWrapperComponent implements OnChanges, AfterViewInit {
  ...
  adjustRootFormProperty(): boolean {
    ...
    const hiddenItemYesNo = rootProperty?.searchProperty('/__$hiddenItemYesNo');
    if (hiddenItemYesNo?.schema) {
      hiddenItemYesNo.schema.readOnly = this.isHiddenRadioItem;
    }
    return ret;
  }

  private setFieldValue(item, path): boolean {
    const rootProperty = this.itemForm?.rootProperty;
    if (item && !Util.isEmpty(item)) {
      // Loading is done. Change of value should emit the value in valueChanged().
      const field = rootProperty?.searchProperty(path);
      if (field) {
  ...
  validateEnableWhenAll(value: any, arrayProperty: ArrayProperty, rootProperty: PropertyGroup) {
    ...
    this.validateEnableWhenDependencies(rootProperty);
  ...
  private validateEnableWhenDependencies(rootProperty: PropertyGroup): any [] {
    if (!rootProperty?.value?.linkId) {
      return;
    }
    const linkIds = this.formService.getNodesHavingEnableWhenDependency(rootProperty.value.linkId);

    if (linkIds && linkIds.length > 0) {
      const warnings = [];
      for (const linkId of linkIds) {
        const warningCode = Util.ENABLEWHEN_DEPENDENCY_WARNING_CODE;
        const warning: any = {};
        warning.code = warningCode;
        warning.message = 'Warning: Displayed item is referenced in the enableWhen.question attribute of the item: ' + linkId;
        warning.linkId = linkId;
        warnings.push(warning)
      }
      this.warningsChanged.next(warnings);
      return warnings;
    }
    this.warningsChanged.next(null);
    return null;
  }
