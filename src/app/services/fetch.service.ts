...
export class FetchService {
  ...
  callGetWithRetry(url: string, nbRetries, delayInSeconds) {
    ...
    const timeDelay = delayInSeconds * 1000;
    return this.http
      .get(url, {headers})
      .pipe(
        retryWhen((errors) =>
          errors.pipe(
            mergeMap((error, retryCount) => {
              if (retryCount >= nbRetries) {
                // After nbRetries retries, throw an error
                return throwError(() => error);
              }
              console.log(`Retrying... Attempt ${retryCount + 1}`);
              return of(null).pipe(delay(timeDelay)); // Wait delayInSeconds seconds before retrying
            })
          )
        ),
        catchError((error) => {
