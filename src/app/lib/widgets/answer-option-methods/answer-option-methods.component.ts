...
export class AnswerOptionMethodsComponent extends LabelRadioComponent implements OnInit, AfterViewInit {
  ...
  constructor(private formService: FormService, private extensionService: ExtensionsService, private liveAnnouncer: LiveAnnouncer) {
    super();
  }
  ...
  handleChange() {
    ...
    switch (this.formProperty.value) {
      case 'answer-option':
        ...
        this.extensionService.removeExtensionsByUrl(ExpressionComponent.ANSWER_EXPRESSION_URL);
        break;
  ...
  updateUI() {
    ...
    const extension = this.extensionService.getFirstExtensionByUrl(ExpressionComponent.ANSWER_EXPRESSION_URL);
