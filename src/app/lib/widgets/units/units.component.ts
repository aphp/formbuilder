  ...
  template: `
    <div [ngClass]="{'row': labelPosition === 'left', 'm-0': true}">
      ...
      <div class="{{controlWidthClass}} p-0">
        <input autocomplete="off" type="text" [attr.id]="elementId" placeholder="Search for UCUM units or type your own" (keydown.enter)='doEnter($event)'
               class="form-control"/>
        <small *ngIf="validateMessage"
               [class]="isWarningMessage?'text-danger':'text-info'" [innerHTML]="validateMessage"></small>
      </div>
    </div>
  `,
  styles: [`
...
export class UnitsComponent extends LfbArrayWidgetComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit() {
    ...
    sub = this.formProperty.valueChanges.subscribe(() => {
      ...
      for (let i = 0, len = initialUnits.length; i < len; ++i) {
        ...
        if (this.options.maxSelect === '*') {
          ...
          const code = initialUnits[i]?.valueCoding?.code;
          this.autoComp.setFieldVal(this.hasNumberDataType() ? (code || dispVal) : dispVal);
        }
      }
    });
    this.subscriptions.push(sub);
    ...
    LForms.Def.Autocompleter.Event.observeListSelections(this.elementId, (data) => {
      ...
      if (data.removed) {
        ...
        if (this.hasNumberDataType()) {
          this.autoComp.setFieldVal(data.item_code);
          const parseResp = UcumLhcUtils.getInstance().validateUnitString(data.final_val, true);
          if (parseResp && parseResp.status === 'valid' && parseResp.unit?.code) {
            this.isWarningMessage = false;
            this.validateMessage = `${parseResp.unit.code} (${parseResp.unit.name}) is a valid unit expression.`;
          }
        }
      } else {
        const unitExt = this.getUnitExtension();
        if (data.final_val && unitExt?.valueCoding?.display !== data.final_val) {
          if (this.hasNumberDataType()) {
            this.updateUnits(this.createUnitExt(UnitsComponent.unitsExtUrl[this.dataType],
              UnitsComponent.ucumSystemUrl, data.final_val, data.final_val));
            const parseResp = UcumLhcUtils.getInstance().validateUnitString(data.final_val, true);
            if (parseResp && parseResp.status === 'valid' && parseResp.unit?.code) {
              this.isWarningMessage = false;
              this.validateMessage = `${parseResp.unit.code} (${parseResp.unit.name}) is a valid unit expression.`;
            }
            if (parseResp && (parseResp.status === 'invalid' || parseResp.status === 'error')) {
              this.validateMessage = parseResp.msg.join(' - ');
      ...
      if (this.hasNumberDataType() && (!data.list || data.list.length === 0)) {
        const parseResp = UcumLhcUtils.getInstance().validateUnitString(data.final_val, true);
        if (this.autoComp && parseResp && parseResp.status === 'valid' && parseResp.unit?.code) {
          this.isWarningMessage = false;
          this.validateMessage = `${parseResp.unit.code} (${parseResp.unit.name}) is a valid unit expression.`;
          this.autoComp.setFieldVal(parseResp.unit.code);
          const unitExt = this.getUnitExtension();
          if (unitExt?.valueCoding?.display !== parseResp.unit.name) {
            this.updateUnits(this.createUnitExt(UnitsComponent.unitsExtUrl[this.dataType],
              UnitsComponent.ucumSystemUrl, parseResp.unit.code, parseResp.unit.code));
          }
        } else if (parseResp && (parseResp.status === 'invalid' || parseResp.status === 'error')) {
          this.validateMessage = parseResp.msg.join(' - ');
          this.isWarningMessage = true;
          this.updateUnits(this.createUnitExt(UnitsComponent.unitsExtUrl[this.dataType],
  ...
  hasNumberDataType() {
    return (this.dataType === 'integer' || this.dataType === 'decimal');
  }
