...
Cypress.Commands.add('toggleTreeNodeExpansion', (text) => {
    const tooltipId = cy.get('div[role="tooltip"]:contains("'+text+'")').invoke('attr', 'id').then((tooltipId) => {
        ...
        cy.get('@expander', { timeout: 4000 }).should('be.visible');
        cy.get('@expander').click({ timeout: 4000 });
...
Cypress.Commands.add('fhirSearch', (titleSearchTerm) => {
    ...
    cy.get('input.form-control[placeholder="Search form identifier"]').type(titleSearchTerm);
    ...
    //cy.contains('lfb-message-dlg > button', 'Close').click();
});
