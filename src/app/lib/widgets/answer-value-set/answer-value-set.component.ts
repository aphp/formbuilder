...
export class AnswerValueSetComponent extends StringComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  showValueSetSearchDialog() {
    // Server picked, invoke search dialog.
    this.formProperty.searchProperty('__$initialExpressionYesNo').setValue('no', false);
    const modalRef = this.modalService.open(ValueSetSearchDlgComponent, {size: 'lg', scrollable: true});
    const questionnaire = this.formService.autoLoadForm();
    modalRef.componentInstance.questionnaireUseContext = questionnaire.useContext;
    modalRef.result.then((url) => {
      if (url !== false) {
        this.formProperty.setValue(url, false);
      }
    }, (reason) => {
