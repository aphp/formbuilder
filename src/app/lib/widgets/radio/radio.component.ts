...
@Component({
  ...
  template: `<div class="widget form-group">
    ...
    <div class="container">
      <div class="row">
        <div *ngFor="let option of schema.oneOf" class="form-check form-check-inline">
          <input class="form-check-input" [formControl]="control" [attr.id]="id + '.' + option.enum[0]" name="{{id}}" (keydown.enter)='doEnter($event)'
        ...
        <div *ngFor="let option of schema.enum" class="form-check form-check-inline" [ngClass]="{col: schema.widget.layout === 'row'}">
          <input class="form-check-input" [formControl]="control" [attr.id]="id + '.' + option" name="{{id}}" (keydown.enter)='doEnter($event)'
	...
	<input *ngIf="schema.readOnly" name="{{name}}" type="hidden" [formControl]="control" (keydown.enter)='doEnter($event)'>
</div>`,
  styles: [`
    lfb-label {
      margin-left: .3rem;
